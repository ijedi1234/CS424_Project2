...
function drawPie(starInfo, name, target) {
  ...
  if (target) {
	...
	// vis = vis.append("svg").attr("width", "1200").attr("height", "800"); // Added height and width so arc is visible
	    ...
	    var transX = (width / 2) + (radius * Math.cos(angle));
	    var transY = (height / 2) + (radius * Math.sin(angle));
	...
	.text(name);
		...
		var transX1 = ((width / 2) + ((outerRad - tabIn) * Math.cos(angle) ) )
		var transY1 = ((height / 2) + ((outerRad - tabIn) * Math.sin(angle) ) )
		var transX2 = ((width / 2) + ((outerRad + tabOut) * Math.cos(angle) ) )
		var transY2 = ((height / 2) + ((outerRad + tabOut) * Math.sin(angle) ) )
